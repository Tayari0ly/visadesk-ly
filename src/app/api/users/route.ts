...
const allowedRoles = ["super_admin", "branch_admin", "supervisor", "employee", "viewer"] as const;
...
export async function GET() {
  try {
    ...
    const list = await db.select({ id: users.id, username: users.username, fullName: users.fullName, role: users.role, branchId: users.branchId, active: users.active, createdAt: users.createdAt }).from(users)
      .where(me.role === "super_admin" || me.role === "admin" ? undefined : me.branchId ? eq(users.branchId, me.branchId) : eq(users.id, me.id)).orderBy(desc(users.createdAt));
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role === "super_admin" && me.role !== "super_admin") return NextResponse.json({ success: false, error: "FORBIDDEN" }, { status: 403 });
    const inserted = await db.insert(users).values({ username, passwordHash: hashPassword(password), fullName: String(body.fullName || "").trim().slice(0, 200), role, branchId: me.role === "super_admin" ? (body.branchId || null) : me.branchId, active: true }).returning({ id: users.id, username: users.username, fullName: users.fullName, role: users.role, branchId: users.branchId, active: users.active });
