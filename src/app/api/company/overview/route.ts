...
export async function GET() {
  try {
    ...
    const [branchRows, userRows, applicationRows] = await Promise.all([
    ...
    return NextResponse.json({ success: true, branch: branchRows[0] || null, summary: { users: Number(userRows[0]?.value || 0), applications: Number(applicationRows[0]?.value || 0) } });
  } catch (error) {
